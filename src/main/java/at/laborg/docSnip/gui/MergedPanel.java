...
import at.laborg.docSnip.DocSnipGUI;
...
public class MergedPanel extends JPanel {
	...
	private final DocSnipGUI docSnip;

	public MergedPanel(PageCluster cluster, DocSnipGUI docSnip) {
		super();
		this.docSnip = docSnip;
	...
	private void alignSelected(Point p) {
		for (DrawableCropRect crop : crops) {
			if (crop.contains(p)) {
				docSnip.alignSelRects(crop.x, crop.y, crop.width, crop.height);
				break;
			}
		}
	}
		...
		public void keyPressed(KeyEvent e) {
			...
			case KeyEvent.VK_DOWN:
				...
				if ((e.getModifiers() & InputEvent.CTRL_MASK) != 0) {
					docSnip.resizeSelRects(x, y);
				} else {
					docSnip.moveSelectedRects(x, y);
				}
				break;
			default:
			}
	...
	private class MergedPanelMouseAdapter extends MouseAdapter implements ActionListener {
		...
		public void mouseDragged(MouseEvent mE) {
			...
			case MOVE_CROP:
				...
				if (mE.isShiftDown()) {
					docSnip.moveSelectedRects(curPoint.x - lastDragPoint.x, curPoint.y - lastDragPoint.y);
				} else {
			...
			case RESIZING_HOTCORNER_LR:
				...
				if (mE.isShiftDown()) {
					docSnip.resizeSelRects(curPoint.x - lastDragPoint.x, curPoint.y - lastDragPoint.y);
				} else {
			...
			case RESIZING_HOTCORNER_UL:
				...
				if (mE.isShiftDown()) {
					docSnip.resizeSelRects(lastDragPoint.x - curPoint.x, lastDragPoint.y - curPoint.y);
					docSnip.moveSelectedRects(curPoint.x - lastDragPoint.x, curPoint.y - lastDragPoint.y);
				} else {
					curPoint.translate(relativeHotCornerGrabDistance.x, relativeHotCornerGrabDistance.y);
