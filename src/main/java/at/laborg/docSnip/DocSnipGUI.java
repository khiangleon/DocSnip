// $Id: DocSnipGUI.java 70 2012-05-26 16:20:19Z laborg $
/**
...
import at.laborg.docSnip.gui.HelpDialog;
import at.laborg.docSnip.gui.MergedPanel;
import at.laborg.docSnip.gui.WrapLayout;
import at.laborg.docSnip.model.ClusterDefinition;
import at.laborg.docSnip.model.CropDefinition;
import at.laborg.docSnip.model.PageCluster;
import at.laborg.docSnip.model.PageExcludes;
import at.laborg.docSnip.model.WorkingSet;
import at.laborg.docSnip.exception.CropException;
import at.laborg.docSnip.utils.ClusterCreator;
import at.laborg.docSnip.utils.ClusterRenderWorker;
import at.laborg.docSnip.utils.DesktopHelper;
import at.laborg.docSnip.utils.DocSnipFileHandling;
import at.laborg.docSnip.utils.DocumentCropper;
import at.laborg.docSnip.utils.FileDrop;
import at.laborg.docSnip.utils.PageNumberParser;
...
public class DocSnipGUI extends JFrame implements PropertyChangeListener, ComponentListener {
    ...
    public DocSnipGUI(String[] args) {
        super(Messages.getString("DocSnipGUI.windowTitle")); //$NON-NLS-1$
        init();
        ...
        if (fileArg.exists() && fileArg.getAbsolutePath().trim().endsWith(".pdf")) { //$NON-NLS-1$
            try {
                ...
                JOptionPane.showMessageDialog(this, e.getMessage(),
                        Messages.getString("DocSnipGUI.docSnipError"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
    ...
    private void init() {
        ...
        this.setTransferHandler(new DocSnipTransferHandler(this));
        ...
        JMenu fileMenu = new JMenu(Messages.getString("DocSnipGUI.file")); //$NON-NLS-1$
        ...
        loadButton = new JMenuItem(Messages.getString("DocSnipGUI.loadFile"), KeyEvent.VK_L); //$NON-NLS-1$
        ...
        exitButton = new JMenuItem(Messages.getString("DocSnipGUI.exit"), KeyEvent.VK_E); //$NON-NLS-1$
        exitButton.addActionListener(a -> System.exit(0));
        fileMenu.add(exitButton);

        openDonationLinkButton = new JMenuItem(Messages.getString("DocSnipGUI.donate")); //$NON-NLS-1$
        ...
        showHelpButton = new JMenuItem(Messages.getString("DocSnipGUI.showHelp")); //$NON-NLS-1$
        showHelpButton.addActionListener(a -> new HelpDialog(this, Messages.getString("DocSnipGUI.docSnipHelp"), Dialog.ModalityType.MODELESS));
    ...
    private void setUILook() {
        try {
            ...
            System.out.println(Messages.getString("DocSnipGUI.lookAndFeelError")); //$NON-NLS-1$
    ...
    private static PageExcludes getExcludedPages() {
        ...
        while (!inputIsValid) {
            String input = JOptionPane.showInputDialog(Messages.getString("DocSnipGUI.excludedPagesInfo"), previousInput);
            previousInput = input;
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, e.getMessage(),
                        Messages.getString("DocSnipGUI.inputError"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
            }
    ...
    private void showSaveFileDialog() {
        Platform.runLater(() -> {
            ...
            fileChooser.setInitialFileName(DocSnipFileHandling.getRecommendedFileName(workingSet.getSourceFile()));
    ...
    private void loadPDF(File file) {
        try {
            ...
            JOptionPane.showMessageDialog(this, e.getMessage(), Messages.getString("DocSnipGUI."), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
        } catch (PdfException e) {
            JOptionPane.showMessageDialog(this, e.getMessage(),
                    Messages.getString("DocSnipGUI.loadingError"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
        }
    }

    private void savePDF(File file) {
        setWorkingState(Messages.getString("DocSnipGUI.loadingPDF")); //$NON-NLS-1$
        try {
            ...
            JOptionPane.showMessageDialog(this, e.getMessage(),
                    Messages.getString("DocSnipGUI.croppingError"), JOptionPane.ERROR_MESSAGE); //$NON-NLS-1$
        } finally {
    ...
    private void showPreview() {
        try {
            setWorkingState(Messages.getString("DocSnipGUI.createShowPreview")); //$NON-NLS-1$
    ...
    private File createAndExecuteCropJobForPreview() throws IOException, DocumentException, CropException {
        File tmpCropFileDestination = File.createTempFile("docSnip", ".pdf"); //$NON-NLS-1$ //$NON-NLS-2$
        CropDefinition cropDefinition = CropDefinition.createCropDefinition(workingSet.getSourceFile(),
    ...
    protected void importNewPdfFile(File loadFile) throws IOException, PdfException {
        ...
        progressBar.setVisible(true);
        progressBar.setString(Messages.getString("DocSnipGUI.loadingNewFile")); //$NON-NLS-1$
    ...
    private void reloadWithOtherExcludes() throws IOException, PdfException {
        previewPanel.removeAll();
        progressBar.setString(Messages.getString("DocSnipGUI.reloadingFile")); //$NON-NLS-1$
    ...
    public void setDefinedSizeSelRects() {
        ...
        String input = JOptionPane.showInputDialog(Messages.getString("DocSnipGUI.size"), defInput);
    ...
    public void setPositionSelRects() {
        ...
        String input = JOptionPane.showInputDialog(Messages.getString("DocSnipGUI.position"), defInput);
    ...
    private void setStateAfterClusteringFinished(ClusterDefinition newClusters, PageExcludes newPageExcludes,
        ...
        progressBar.setString(Messages.getString("DocSnipGUI.clusteringRenderingFinished")); //$NON-NLS-1$
