...
import at.laborg.docSnip.model.SplitFinder;
import at.laborg.docSnip.model.CropFinder;
	...
	private static class CommandValues {
		...
		private static boolean isValidJob(final CommandValues job) {
			...
			if (job.getDestFile() == null) {
				File recommendedDest = DocSnipFileHandling.getRecommendedDestination(job.getSourceFile());
			...
			try {
				DocSnipFileHandling.checkValidStateAndCreate(job.getDestFile());
			} catch (IllegalArgumentException e) {
